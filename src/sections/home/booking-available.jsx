...
export function BookingAvailable({ title, subheader, chart, ...other }) {
  ...
  const chartSeries = (chart.series.filter((i) => i.label === 'Completed Days')[0].value / total) * 100;
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          name: { offsetY: -12 },
          value: { offsetY: 6 },
          total: { label: 'Days', formatter: () => fNumber(total) },
        },
      },
    },
    ...chart.options,
          ...
          >
            <Box
              sx={{
                ...
                ...(item.label === 'Completed Days' && { bgcolor: chartColors[1] }),
              }}
            />
